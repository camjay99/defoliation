...
def preprocess_HLS(start_date, end_date, geometry, phenology):
    # Load HLS S30/L30
    ...
    collection_S30 = (ee.ImageCollection("NASA/HLS/HLSS30/v002")
                        .filterDate('2023-05-25', '2023-07-28')
                        .filterBounds(geometry))

    # Rename bands so collections can be used together.
    def rename_bands_L30(image):
        return image.select(['B2','B4','B5'],['BLUE', 'RED', 'NIR'])

    def rename_bands_S30(image):
        return image.select(['B2','B4','B8'],['BLUE', 'RED', 'NIR'])
        
    def preprocess(image):
        # New Bands
        EVI = image_s.expression(
            '2.5 * ((NIR-RED) / (NIR + 6 * RED - 7.5* BLUE + 1))', {
                'NIR': image_s.select('NIR'),
                'RED': image_s.select('RED'),
                'BLUE': image_s.select('BLUE')
            }).rename('EVI')
        doy = image.date().getRelative('day', 'year').add(1)
        
        # Masks
        cloud_mask_val = ee.Number.parse("11101010", 2)
        cloud_mask = image.select("Fmask").bitwiseAnd(cloud_mask_val).eq(0)
        EVI_mask = EVI.lte(1).And(EVI.gte(0))
        pheno_mask = doy_band.gte(phenology.select('SoS')).And(doy_band.lte(phenology.select('EoS')))
    ...
    collection_S30 = collection_S30.map(lambda image: preprocess(rename_bands_L30(image)))
