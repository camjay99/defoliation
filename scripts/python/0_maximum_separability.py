...
if args.cloudstorage:
    ...
    file_name_prefix = f'average_phenology_{name}/average_phenology_{args.data}'
    image_manifests = {}
...
for i in range(gridSize):
    ...
    def window_smooth(day):
        ...
        doy_ims = col.filter(
            ee.Filter.dayOfYear(
                day.subtract(args.smooth_window).add(365).mod(365), 
    ...
    if args.submit:
        if args.cloudstorage:
            ...
            image_manifests[i] = {
                ...
                'tilesets': [
                    {'id': '0', 'sources': [ {'uris': [f'gs://{args.bucket}/{file_name_prefix}.tif']}]}
                ],
